...
def plot_training_vs_test_metrics(val_results, test_results, model_name):
    ...
    val_values = [
        val_results.get('dice', 0.9579),  # Your validation results
        val_results.get('jaccard_coeff', 0.9191),
        val_results.get('accuracy_spheroid', 0.9895)
    ]
    test_values = [
        test_results['dice_mean'],
        test_results['iou_mean'],
        test_results['accuracy_mean']
    ]
    test_stds = [
        test_results['dice_std'],
        test_results['iou_std'],
        test_results['accuracy_std']
    ]
    
    x = np.arange(len(metrics))
    width = 0.35
    
    for idx, (ax, metric, val_val, test_val, test_std) in enumerate(zip(
        axes, metrics, val_values, test_values, test_stds)):
        
        bars1 = ax.bar(x[idx] - width/2, val_val, width, label='Validation', 
                      color='steelblue', alpha=0.8)
        bars2 = ax.bar(x[idx] + width/2, test_val, width, label='Test', 
                      color='coral', alpha=0.8, yerr=test_std, capsize=5)
        ...
        # Add value labels
        ax.text(x[idx] - width/2, val_val + 0.01, f'{val_val:.3f}', 
               ha='center', va='bottom', fontsize=10, fontweight='bold')
        ax.text(x[idx] + width/2, test_val + test_std + 0.01, f'{test_val:.3f}', 
               ha='center', va='bottom', fontsize=10, fontweight='bold')
