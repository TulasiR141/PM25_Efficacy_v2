...
(base) ┌──(vertico㉿vertico)-[~/thws/semester2/project/src_new]
└─$ cat bring_augmented_base_images.py 
import os
import shutil
...
# Paths
# ------------------------------------------------------------
# Base dir = project root (one level up from src_new)
BASE_DIR = os.path.abspath(os.path.join(os.path.dirname(__file__), ".."))

# Source: YOLO augmented dataset
SRC_BASE = os.path.join(BASE_DIR, "data", "consolidated_yolo_augmented_v1")

# Target: consolidated directory you already created earlier
DST_BASE = os.path.join(BASE_DIR, "data_new", "echo_v3_consolidated")
IMG_OUT = os.path.join(DST_BASE, "images")
LBL_OUT = os.path.join(DST_BASE, "labels")
...
# Suffixes that indicate augmented versions we want to SKIP
AUG_SUFFIXES = (
    "_hflip",
    "_vflip",
    "_rot90",
...
def is_augmented_stem(stem: str) -> bool:
    """
    Returns True if the filename stem ends with a known augmentation suffix.
    Example:
      stem = "A_0011_jpg.rf.284c82..._hflip" -> True
      stem = "A_0011_jpg.rf.284c82..."       -> False
    """
...
def main():
    ...
    print(f"📂 Consolidated dir: {DST_BASE}\n")
    ...
    for split in SPLITS:
        ...
        if not os.path.isdir(img_dir) or not os.path.isdir(lbl_dir):
            print(f"⚠ Missing images/labels dir for split '{split}', skipping.")
            continue
        ...
        for fname in os.listdir(img_dir):
            ...
            # Skip augmented variants like *_hflip, *_rot90, etc.
            if is_augmented_stem(stem):
                # Uncomment if you want to see what gets skipped:
                # print(f"  ⏭ Skipping augmented image: {fname}")
                continue

            # Expected label name is stem + ".txt"
            label_name = stem + ".txt"
            label_path = os.path.join(lbl_dir, label_name)

            if not os.path.isfile(label_path):
                print(f"  ⚠ No label for base image '{fname}', skipping.")
                continue
            ...
            if os.path.exists(dst_img) or os.path.exists(dst_lbl):
                # You can choose to overwrite instead by removing this check
                print(f"  ℹ File(s) already exist for '{fname}', skipping copy.")
                continue
    ...
    print("🎉 Finished copying base images from YOLO augmented dataset.")
    print(f"📸 Total base images copied: {total_images}")
    print(f"📝 Total labels copied:      {total_labels}")
    print(f"📦 Output in: {DST_BASE}")
