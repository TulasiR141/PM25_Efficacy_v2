...
Notes:
- Uses union-of-instances for prediction mask (binary).
- Uses union-of-polygons for GT mask (binary).
"""
...
def combine_pred_masks(result, h: int, w: int) -> np.ndarray:
    """
    Combine all predicted instance masks into one union boolean mask (H,W).
    """
    if result.masks is None or result.masks.data is None:
        return np.zeros((h, w), dtype=bool)

    m = result.masks.data  # torch tensor: (N,H,W)
    m_np = m.detach().cpu().numpy().astype(np.float32)
    if m_np.ndim != 3 or m_np.shape[0] == 0:
        return np.zeros((h, w), dtype=bool)

    union = (m_np > 0.5).any(axis=0)
...
def find_weights(model_dir: Path) -> Path:
    ...
    # Fast path: standard Ultralytics run layout
    ...
    # Fallback: recursive search (handles weird nesting)
...
def main():
    ...
    ap.add_argument(
        "--model_dir",
        type=str,
        default="yolo_training_output/yolov8n_seg_spheroids_softboundary_v27",
        help="Path to the YOLO training run directory (expects weights/best.pt).",
    )
    ap.add_argument(
        "--test_dir",
        type=str,
        default="data_new/yolo_splits_v1/test",
    ...
    for idx, img_path in enumerate(image_paths, start=1):
        ...
        result = results[0]
        pr = combine_pred_masks(result, h, w)
